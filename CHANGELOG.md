...
- `PackageArchive` (`kind=archive`): the results archive (1968–2022) as
  yearly exports — `period` is the bare year `YYYY`. Rides the tape
  entitlement, not ULTRA. Completes the spec's four package kinds.

### Fixed

- The `HistoryPackagesParams.Kind` doc no longer claims every non-tape kind
  needs ULTRA — only rally and rankings do; archive is gated like tape.
